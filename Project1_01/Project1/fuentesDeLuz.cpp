...
// Positions of the point lights posiciones de luz puntual 
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Directional light sol si modificamos los valores se hara mas oscura la escena en general 
		...
		// SpotLight esto se movera con base al mouse con la camara
		...
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform1i(glGetUniformLocation(lightingShader.Program, "transparency"), 1.0);//se modifica 
		Dog.Draw(lightingShader);
		glDisable(GL_BLEND);  //Desactiva el canal alfa esto es porque gasta muchos recursos
		glBindVertexArray(0);

...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, 0.5f, glm::vec3(2.5f, 3.5f, 5.0f)); // use to compare orthographic and perspective projection
		...
		//
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(2.5f, 5.0f,1.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(3.0f, 2.0f, 3.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.2f, 2.2f, 3.2f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glBindVertexArray(0);

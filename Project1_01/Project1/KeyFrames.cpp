...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Animacion por keyframe", nullptr, nullptr);
...
{
//Dog Controls
const float MAX_ROTATION = 10.0f;   // Límite positivo
const float MIN_ROTATION = -10.0f;  // Límite negativo

//cabeza
	if (keys[GLFW_KEY_0])
	{
		head += 1.5f;
		if (head > MAX_ROTATION)
		{
			head = MAX_ROTATION;
		}
	}
